...
## Summary
Six participants were asked to perform Unilateral Dumbbell Biceps Curl in five different manners (Class A, B, C, D, E).
Class A corresponds to the correct execution of the exercise while the other 4 classes are incorrect executions.
Data was collected using accelerometers attached to the participants and dumbbells. 
More information is available from http://groupware.les.inf.puc-rio.br/har (see the section on the Weight Lifting Exercise Dataset).

The objective of the project is to predict the manner which the participants performed the exericse.

* Some exploratory data analysis is performed to understand the dataset. 
* The data is santized to handle irrelevant attributes and missing values.
* Random Forest is selected to build the predictor. 
* The predictor is evaluated to be highly accurate on both in-sample and out-of-sample observations.
* The predictor is applied onto the test dataset.
...
## Data Preprocessing
* Sparse attributes (>= 90% null values) are dropped from the datasets. They provide minimal or no information.
* The remaining attributes do not have low variance. 
* The dropped columns (from training dataset) are removed from the test dataset.
* The datasets are reduced from 160 attributes to 60 attributes (including first 7 supporting attributes). 
...
* Random forests is used to build the model.
    ...
    + Features are randomly sampled (implicit feature selection).
